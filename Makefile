...
SRC_SAT_C = src/main_satellite.c src/network.c src/sensor.c src/command.c
SRC_GND_C = src/main_ground.c src/network.c src/command.c
...
# 위성 프로그램 빌드
satellite: $(SRC_SAT_C) $(SRC_GND_CPP)
	$(CC) $(CFLAGS) $(SRC_GND_C) -o $(TARGET_GND) \
	$(if $(SRC_GND_CPP),$(CXXFLAGS) $(SRC_GND_CPP))
